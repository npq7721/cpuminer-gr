...
static bool stratum_parse_extranonce(struct stratum_ctx *sctx, json_t *params,
  ...
  if (!opt_quiet && !sctx->dev) /* pool dynamic change */
    applog(LOG_INFO, "Stratum extranonce1= %s, extranonce2 size= %d", xnonce1,
           xn2_size);
  //   if (pndx == 0 && opt_debug)
  //		applog(LOG_DEBUG, "Stratum set nonce %s with extranonce2
  // size=%d", 			xnonce1, xn2_size);
...
bool stratum_authorize(struct stratum_ctx *sctx, const char *user,
  ...
  if (sret) {
    ...
    if (!extra) {
      ...
      //			if (opt_debug && (!res_val ||
      // json_is_false(res_val))) applog(LOG_DEBUG, "extranonce subscribe not
      // supported");
      json_decref(extra);
    }
    free(sret);
